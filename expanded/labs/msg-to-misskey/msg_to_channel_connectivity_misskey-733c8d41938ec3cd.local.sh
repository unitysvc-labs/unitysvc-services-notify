#!/bin/bash
# Connectivity test for a UnitySVC Notify gateway-transformer channel (misskey).
...
#                   the gateway transforms it into the misskey-native payload and
...
URL="https://mock.unitysvc.dev/misskey/api/notes/create"
status=$(curl -sS --max-time 10 \
    -X POST \
    -H "Content-Type: application/json" \
    -d '{"i": "test_token", "text": "connectivity check", "visibility": "public"}' \
