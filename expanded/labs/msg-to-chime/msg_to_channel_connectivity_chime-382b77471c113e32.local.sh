#!/bin/bash
# Connectivity test for a UnitySVC Notify gateway-transformer channel (chime).
...
#                   the gateway transforms it into the chime-native payload and
...
URL="https://mock.unitysvc.dev/chime/incomingwebhooks/demo123?token=demo123"
status=$(curl -sS --max-time 10 \
    -X POST \
    -H "Content-Type: application/json" \
    -d '{"Content": "connectivity check"}' \
