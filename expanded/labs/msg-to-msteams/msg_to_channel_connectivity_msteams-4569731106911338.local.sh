#!/bin/bash
# Connectivity test for a UnitySVC Notify gateway-transformer channel (msteams).
...
#                   the gateway transforms it into the msteams-native payload and
...
URL="https://mock.unitysvc.dev/msteams/webhookb2/demo"
status=$(curl -sS --max-time 10 \
    -X POST \
    -H "Content-Type: application/json" \
    -d '{"@type":"MessageCard","@context":"https://schema.org/extensions","text":"connectivity check"}' \
