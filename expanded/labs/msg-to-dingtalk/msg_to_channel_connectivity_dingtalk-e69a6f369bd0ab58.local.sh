#!/bin/bash
# Connectivity test for a UnitySVC Notify gateway-transformer channel (dingtalk).
...
#                   the gateway transforms it into the dingtalk-native payload and
...
URL="https://mock.unitysvc.dev/dingtalk/robot/send?access_token=demo123"
status=$(curl -sS --max-time 10 \
    -X POST \
    -H "Content-Type: application/json" \
    -d '{"msgtype": "text", "at": {"atMobiles": [], "isAtAll": false}, "text": {"content": "connectivity check"}}' \
