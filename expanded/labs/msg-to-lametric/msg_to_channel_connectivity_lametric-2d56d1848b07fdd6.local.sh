#!/bin/bash
# Connectivity test for a UnitySVC Notify gateway-transformer channel (lametric).
...
#                   the gateway transforms it into the lametric-native payload and
...
URL="https://mock.unitysvc.dev/lametric/api/v2/device/notifications"
status=$(curl -sS --max-time 10 \
    -X POST \
    -H "Content-Type: application/json" \
    -d '{"priority": "info", "icon_type": "none", "lifetime": 120000, "model": {"cycles": 1, "frames": [{"icon": 1, "text": "connectivity check"}]}}' \
