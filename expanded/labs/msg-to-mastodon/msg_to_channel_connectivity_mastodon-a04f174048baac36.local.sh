#!/bin/bash
# Connectivity test for a UnitySVC Notify gateway-transformer channel (mastodon).
...
#                   the gateway transforms it into the mastodon-native payload and
...
URL="https://mock.unitysvc.dev/mastodon/api/v1/statuses"
status=$(curl -sS --max-time 10 \
    -X POST \
    -H "Content-Type: application/json" \
    -d '{"status":"connectivity check"}' \
