#!/bin/bash
# Connectivity test for a UnitySVC Notify gateway-transformer channel (pushover).
...
#                   the gateway transforms it into the pushover-native payload and
...
URL="https://mock.unitysvc.dev/pushover/1/messages.json"
status=$(curl -sS --max-time 10 \
    -X POST \
    -H "Content-Type: application/json" \
    -d '{"token":"test_key","user":"test_user","message":"connectivity check"}' \
