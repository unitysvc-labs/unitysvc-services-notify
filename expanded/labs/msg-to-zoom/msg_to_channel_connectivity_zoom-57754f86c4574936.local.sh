#!/bin/bash
# Connectivity test for a UnitySVC Notify gateway-transformer channel (zoom).
...
#                   the gateway transforms it into the zoom-native payload and
...
URL="https://mock.unitysvc.dev/zoom/incoming/hook/demo"
status=$(curl -sS --max-time 10 \
    -X POST \
    -H "Content-Type: application/json" \
    -d '{"content":{"head":{"text":"connectivity check"},"body":[{"type":"message","text":"ping"}]}}' \
