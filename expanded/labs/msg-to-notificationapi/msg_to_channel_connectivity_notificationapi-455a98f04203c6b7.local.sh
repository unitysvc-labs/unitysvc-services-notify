#!/bin/bash
# Connectivity test for a UnitySVC Notify gateway-transformer channel (notificationapi).
...
#                   the gateway transforms it into the notificationapi-native payload and
...
URL="https://mock.unitysvc.dev/notificationapi/democlientid/sender"
status=$(curl -sS --max-time 10 \
    -X POST \
    -H "Content-Type: application/json" \
    -d '{"type":"apprise","sms":{"message":"connectivity check"},"to":{"id":"test_id","number":"15551234567"}}' \
