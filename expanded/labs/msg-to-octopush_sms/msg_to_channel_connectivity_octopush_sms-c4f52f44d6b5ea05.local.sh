#!/bin/bash
# Connectivity test for a UnitySVC Notify gateway-transformer channel (octopush-sms).
#   local_testing : POST the channel-NATIVE octopush-sms body (baked in) straight to
#                   a mock upstream (local_url).  No gateway to transform it, no
#                   platform auth.
#   gateway       : POST the canonical envelope to service_base_url@<channel>;
#                   the gateway transforms it into the octopush-sms-native payload and
#                   forwards it (Bearer auth).
# The octopush-sms-native body is baked into this variant — only the local_url (mock
# upstream) and channel (transformer selector) parameters are supplied.
set -o pipefail


URL="https://mock.unitysvc.dev/octopush_sms/v1/public/sms-campaign/send"
status=$(curl -sS --max-time 10 \
    -X POST \
    -H "Content-Type: application/json" \
    -d '{"recipients":[{"phone_number":"+15551234567"}],"text":"connectivity check","type":"sms_premium","purpose":"alert","sender":"src","with_replies":false}' \
    -o /dev/null -w "%{http_code}" \
    "$URL" 2>/dev/null)


case "$status" in
  2??) echo "connectivity ok (HTTP $status)"; exit 0 ;;
  000) echo "connectivity failed: could not connect to $URL" >&2; exit 1 ;;
  *)   echo "connectivity failed: HTTP $status from $URL" >&2; exit 1 ;;
esac