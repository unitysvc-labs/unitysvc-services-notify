#!/bin/bash
# Connectivity test for a UnitySVC Notify gateway-transformer channel (plivo-sms).
...
#                   the gateway transforms it into the plivo-sms-native payload and
...
URL="https://demoauthid:demo123@mock.unitysvc.dev/plivo_sms/v1/Account/demoauthid/Message/"
status=$(curl -sS --max-time 10 \
    -X POST \
    -H "Content-Type: application/json" \
    -d '{"src": "source", "dst": "+15551234567", "text": "connectivity check"}' \
