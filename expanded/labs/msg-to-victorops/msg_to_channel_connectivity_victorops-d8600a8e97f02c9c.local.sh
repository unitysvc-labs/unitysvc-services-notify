#!/bin/bash
# Connectivity test for a UnitySVC Notify gateway-transformer channel (victorops).
...
#                   the gateway transforms it into the victorops-native payload and
...
URL="https://mock.unitysvc.dev/victorops/integrations/generic/apikey123abc/alert/routingkey"
status=$(curl -sS --max-time 10 \
    -X POST \
    -H "Content-Type: application/json" \
    -d '{"message_type":"CRITICAL","entity_display_name":"connectivity check","state_message":"ping"}' \
