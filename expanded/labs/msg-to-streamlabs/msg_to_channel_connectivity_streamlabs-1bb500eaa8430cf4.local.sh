#!/bin/bash
# Connectivity test for a UnitySVC Notify gateway-transformer channel (streamlabs).
...
#                   the gateway transforms it into the streamlabs-native payload and
...
URL="https://mock.unitysvc.dev/streamlabs/api/v1.0/alerts"
status=$(curl -sS --max-time 10 \
    -X POST \
    -H "Content-Type: application/x-www-form-urlencoded" \
    --data-urlencode 'access_token=test_token' \
    --data-urlencode 'type=donation' \
    --data-urlencode 'message=connectivity check' \
    --data-urlencode 'user_massage=ping' \
