#!/bin/bash
# Connectivity test for a UnitySVC Notify gateway-transformer channel (gotify).
...
#                   the gateway transforms it into the gotify-native payload and
...
URL="https://mock.unitysvc.dev/gotify/message?token=demo123"
status=$(curl -sS --max-time 10 \
    -X POST \
    -H "Content-Type: application/json" \
    -d '{"message":"connectivity check","priority":5}' \
