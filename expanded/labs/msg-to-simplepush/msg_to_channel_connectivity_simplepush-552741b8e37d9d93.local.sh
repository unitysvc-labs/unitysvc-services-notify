#!/bin/bash
# Connectivity test for a UnitySVC Notify gateway-transformer channel (simplepush).
...
#                   the gateway transforms it into the simplepush-native payload and
...
URL="https://mock.unitysvc.dev/simplepush/send"
status=$(curl -sS --max-time 10 \
    -X POST \
    -H "Content-Type: application/json" \
    -d '{"msg":"connectivity check","event":"test"}' \
