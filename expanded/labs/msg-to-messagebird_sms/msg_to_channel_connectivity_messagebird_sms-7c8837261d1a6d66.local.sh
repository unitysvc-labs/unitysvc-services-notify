#!/bin/bash
# Connectivity test for a UnitySVC Notify gateway-transformer channel (messagebird-sms).
...
#                   the gateway transforms it into the messagebird-sms-native payload and
...
URL="https://mock.unitysvc.dev/messagebird/messages"
status=$(curl -sS --max-time 10 \
    -X POST \
    -H "Content-Type: application/json" \
    -d '{"recipients":["31612345678"],"originator":"UnitySVC","body":"connectivity check"}' \
