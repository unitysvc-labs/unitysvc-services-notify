#!/bin/bash
# Connectivity test for a UnitySVC Notify gateway-transformer channel (homeassistant).
...
#                   the gateway transforms it into the homeassistant-native payload and
...
URL="https://mock.unitysvc.dev/homeassistant/api/services/persistent_notification/create"
status=$(curl -sS --max-time 10 \
    -X POST \
    -H "Content-Type: application/json" \
    -d '{"title":"connectivity check","message":"ping"}' \
