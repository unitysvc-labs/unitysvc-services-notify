#!/bin/bash
# Connectivity test for a UnitySVC Notify gateway-transformer channel (pushed).
...
#                   the gateway transforms it into the pushed-native payload and
...
URL="https://mock.unitysvc.dev/pushed/1/push"
status=$(curl -sS --max-time 10 \
    -X POST \
    -H "Content-Type: application/json" \
    -d '{"app_key": "test_key", "app_secret": "test_secret", "target_type": "app", "content": "connectivity check"}' \
