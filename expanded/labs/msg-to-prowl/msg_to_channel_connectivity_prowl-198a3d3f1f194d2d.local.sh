#!/bin/bash
# Connectivity test for a UnitySVC Notify gateway-transformer channel (prowl).
...
#                   the gateway transforms it into the prowl-native payload and
...
URL="https://mock.unitysvc.dev/prowl/publicapi/add"
status=$(curl -sS --max-time 10 \
    -X POST \
    -H "Content-Type: application/json" \
    -d '{"apikey":"test_key","application":"UnitySVC","event":"connectivity check","description":"ping","priority":0}' \
