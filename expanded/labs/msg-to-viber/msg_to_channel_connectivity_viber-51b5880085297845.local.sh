#!/bin/bash
# Connectivity test for a UnitySVC Notify gateway-transformer channel (viber).
...
#                   the gateway transforms it into the viber-native payload and
...
URL="https://mock.unitysvc.dev/viber/pa/send_message"
status=$(curl -sS --max-time 10 \
    -X POST \
    -H "Content-Type: application/json" \
    -d '{"receiver": "target_id", "sender": {"name": "SOURCE"}, "type": "text", "text": "connectivity check"}' \
