#!/bin/bash
# Connectivity test for a UnitySVC Notify gateway-transformer channel (resend-email).
...
#                   the gateway transforms it into the resend-email-native payload and
...
URL="https://mock.unitysvc.dev/resend/emails"
status=$(curl -sS --max-time 10 \
    -X POST \
    -H "Content-Type: application/json" \
    -d '{"from":"test@example.com","to":["test@example.com"],"subject":"connectivity check","text":"ping"}' \
