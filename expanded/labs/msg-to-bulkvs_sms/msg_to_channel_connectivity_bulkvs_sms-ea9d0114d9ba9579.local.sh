#!/bin/bash
# Connectivity test for a UnitySVC Notify gateway-transformer channel (bulkvs-sms).
...
#                   the gateway transforms it into the bulkvs-sms-native payload and
...
URL="https://demo123:demo123@mock.unitysvc.dev/bulkvs_sms/api/v1.0/messageSend"
status=$(curl -sS --max-time 10 \
    -X POST \
    -H "Content-Type: application/json" \
    -d '{"From": "source", "To": "+15551234567", "Message": "connectivity check"}' \
