#!/bin/bash
# Connectivity test for a UnitySVC Notify gateway-transformer channel (pushjet).
...
#                   the gateway transforms it into the pushjet-native payload and
...
URL="https://mock.unitysvc.dev/pushjet/message?secret=demo123"
status=$(curl -sS --max-time 10 \
    -X POST \
    -H "Content-Type: application/json" \
    -d '{"message":"connectivity check","priority":5,"token":"test_key"}' \
