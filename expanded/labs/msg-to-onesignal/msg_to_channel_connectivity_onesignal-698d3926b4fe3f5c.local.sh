#!/bin/bash
# Connectivity test for a UnitySVC Notify gateway-transformer channel (onesignal).
...
#                   the gateway transforms it into the onesignal-native payload and
...
URL="https://mock.unitysvc.dev/onesignal/api/v1/notifications"
status=$(curl -sS --max-time 10 \
    -X POST \
    -H "Content-Type: application/json" \
    -d '{"app_id":"test_id","included_segments":["All"],"headings":{"en":"connectivity check"},"contents":{"en":"ping"}}' \
