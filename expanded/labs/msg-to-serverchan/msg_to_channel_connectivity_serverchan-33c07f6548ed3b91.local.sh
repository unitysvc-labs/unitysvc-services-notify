#!/bin/bash
# Connectivity test for a UnitySVC Notify gateway-transformer channel (serverchan).
...
#                   the gateway transforms it into the serverchan-native payload and
...
URL="https://mock.unitysvc.dev/serverchan/demotoken.send"
status=$(curl -sS --max-time 10 \
    -X POST \
    -H "Content-Type: application/json" \
    -d '{"title":"connectivity check","desp":"ping"}' \
