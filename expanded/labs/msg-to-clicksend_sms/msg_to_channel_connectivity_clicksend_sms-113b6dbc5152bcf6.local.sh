#!/bin/bash
# Connectivity test for a UnitySVC Notify gateway-transformer channel (clicksend-sms).
...
#                   the gateway transforms it into the clicksend-sms-native payload and
...
URL="https://mock.unitysvc.dev/clicksend_sms/v3/sms/send"
status=$(curl -sS --max-time 10 \
    -X POST \
    -H "Content-Type: application/json" \
    -d '{"messages":[{"source":"php","body":"connectivity check","to":"+15551234567"}]}' \
