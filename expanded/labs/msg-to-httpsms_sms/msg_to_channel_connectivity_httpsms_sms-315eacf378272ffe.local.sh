#!/bin/bash
# Connectivity test for a UnitySVC Notify gateway-transformer channel (httpsms-sms).
...
#                   the gateway transforms it into the httpsms-sms-native payload and
...
URL="https://mock.unitysvc.dev/httpsms_sms/v1/messages/send"
status=$(curl -sS --max-time 10 \
    -X POST \
    -H "Content-Type: application/json" \
    -d '{"from": "+15551234567", "to": "+15551234567", "content": "connectivity check"}' \
