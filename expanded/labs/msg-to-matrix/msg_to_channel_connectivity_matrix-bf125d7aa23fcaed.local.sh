#!/bin/bash
# Connectivity test for a UnitySVC Notify gateway-transformer channel (matrix).
...
#                   the gateway transforms it into the matrix-native payload and
...
URL="https://mock.unitysvc.dev/matrix/_matrix/client/v3/rooms/demo/send/m.room.message/1"
status=$(curl -sS --max-time 10 \
    -X POST \
    -H "Content-Type: application/json" \
    -d '{"msgtype":"m.text","body":"connectivity check"}' \
