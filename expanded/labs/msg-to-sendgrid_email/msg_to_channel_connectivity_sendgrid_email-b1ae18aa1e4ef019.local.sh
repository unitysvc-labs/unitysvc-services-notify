#!/bin/bash
# Connectivity test for a UnitySVC Notify gateway-transformer channel (sendgrid-email).
...
#                   the gateway transforms it into the sendgrid-email-native payload and
...
URL="https://mock.unitysvc.dev/sendgrid/v3/mail/send"
status=$(curl -sS --max-time 10 \
    -X POST \
    -H "Content-Type: application/json" \
    -d '{"personalizations":[{"to":[{"email":"test@example.com"}]}],"from":{"email":"test@example.com"},"subject":"connectivity check","content":[{"type":"text/plain","value":"ping"}]}' \
