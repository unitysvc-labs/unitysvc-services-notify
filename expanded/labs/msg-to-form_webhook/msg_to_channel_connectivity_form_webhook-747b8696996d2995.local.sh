#!/bin/bash
# Connectivity test for a UnitySVC Notify gateway-transformer channel (form-webhook).
...
#                   the gateway transforms it into the form-webhook-native payload and
...
URL="https://mock.unitysvc.dev/form"
status=$(curl -sS --max-time 10 \
    -X POST \
    -H "Content-Type: application/x-www-form-urlencoded" \
    --data-urlencode 'version=1.0' \
    --data-urlencode 'title=connectivity check' \
