#!/bin/bash
# Connectivity test for a UnitySVC Notify gateway-transformer channel (spugpush).
...
#                   the gateway transforms it into the spugpush-native payload and
...
URL="https://mock.unitysvc.dev/spugpush/send/dkryFMT07elszGNU18fmtAHOV29gnuBI"
status=$(curl -sS --max-time 10 \
    -X POST \
    -H "Content-Type: application/json" \
    -d '{"title":"connectivity check","content":"ping"}' \
